...
class InsightsDataManager {
    ...
    func updateInsights(_ completion: ((Bool, [OCKInsightItem]?) -> Void)?) {
        ...
        DispatchQueue.global(qos: DispatchQoS.QoSClass.default).async {
            ...
            self.fetchActivityResultsFor(pulseActivity, startDate: startDateComponents, endDate: endDateComponents) {
                (fetchedData) in
                self.pulseData = fetchedData
            }
            guard let temperatureActivity = self.findActivityWith(ActivityIdentifier.temperature) else { return }
            self.fetchActivityResultsFor(temperatureActivity, startDate: startDateComponents, endDate: endDateComponents) { (fetchedData) in
