...
class FoodViewController: UITableViewController, FoodPickerViewControllerDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        NotificationCenter.default.addObserver(self, selector: #selector(FoodViewController.updateCalories), name: Notification.Name.UIApplicationDidBecomeActive, object: nil)
    }
    ...
    override func viewDidLoad() {
        ...
        self.title = "Meal"
        self.foodItems = []
        let addBarButton: UIBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.add, target: self, action: #selector(FoodViewController.pickFood(sender:)));
    ...
    private func addFoodItem(_ foodItem: FoodItem) {
        ...
        print(foodCorrelationForFoodItem)
        let completion: (Bool, Error?) -> Void = {
            [unowned self] (success, error) -> Void in
            DispatchQueue.main.async {
                if !success {
                    print("Error savings calories...")
                    return
                }
